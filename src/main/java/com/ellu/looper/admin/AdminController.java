...
public class AdminController {
  ...
  public ResponseEntity<?> initializeTemplates() {
    Map<NotificationType, String> templates =
        Map.of(
            ...
            "{project}프로젝트에 새로운 {schedule}일정이 추가되었습니다.",
            NotificationType.SCHEDULE_UPDATED,
            "{project}프로젝트에 {schedule}일정이 업데이트되었습니다.",
    ...
    for (Map.Entry<NotificationType, String> entry : templates.entrySet()) {
      if (!templateRepository.existsByType(entry.getKey())) {
        NotificationTemplate template =
            NotificationTemplate.builder().type(entry.getKey()).template(entry.getValue()).build();
