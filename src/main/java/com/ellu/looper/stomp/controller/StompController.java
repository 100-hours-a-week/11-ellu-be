...
public class StompController {
  ...
  public void handleScheduleUpdate(@DestinationVariable Long projectId,
    ...
    ProjectScheduleResponse projectScheduleResponse = projectScheduleService.updateSchedule(
        projectId, userId, scheduleUpdateRequest);

    // Kafka schedule 토픽에 일정 변경 이벤트 발행 (다중 인스턴스 WebSocket 브로드캐스트용)
    ScheduleEventMessage updateEvent = ScheduleEventMessage.builder()
  ...
  public void handleScheduleDelete(@DestinationVariable Long projectId,
    ...
    ScheduleEventMessage deleteEvent = ScheduleEventMessage.builder()
        .projectId(projectId.toString())
        .type(NotificationType.SCHEDULE_DELETED.name())
        .schedule(ScheduleEventMessage.ScheduleDto.builder()
            .id(scheduleId)
            .build())
        .build();
  ...
  public void createSchedule(@DestinationVariable Long projectId,
    ...
    for (ProjectScheduleResponse response : createdList) {
      ScheduleEventMessage event = ScheduleEventMessage.builder()
          .projectId(projectId.toString())
  ...
  public void takeSchedule(@DestinationVariable Long projectId,
    ...
    // 반영된 스케줄 정보 조회
    ProjectSchedule updatedSchedule = projectScheduleRepository.findByIdAndDeletedAtIsNull(
            takeRequest.projectScheduleId())
