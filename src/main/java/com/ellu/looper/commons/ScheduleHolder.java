...
public class ScheduleHolder {
  ...
  public void register(Long projectId, Long userId,
    ...
    waitingClients.put(key, result);

    // 비동기 처리 시작
    CompletableFuture.runAsync(() -> {
      try {
        log.info("[ScheduleHolder] Starting async schedule creation for key: {}", key);
        List<ProjectScheduleResponse> schedules = scheduleService.createSchedules(projectId, userId, request);
        complete(projectId, userId, schedules);
      } catch (Exception e) {
        log.error("[ScheduleHolder] Error during async schedule creation for key: {}, error: {}", key, e.getMessage(), e);
  ...
  public void completeWithError(Long projectId, Long userId, Throwable error) {
    String key = generateKey(projectId, userId);
    log.error("[ScheduleHolder] Completing with error for key: {}, error: {}", key, error.getMessage(), error);
    DeferredResult<ResponseEntity<?>> result = waitingClients.remove(key);
    if (result != null) {
      result.setResult(ResponseEntity.internalServerError().body(
