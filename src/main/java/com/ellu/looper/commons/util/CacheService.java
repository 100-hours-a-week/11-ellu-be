...
public class CacheService {
  ...
  /**
   * Gets a value from cache with locking pattern.
   * If cache miss occurs, the first request locks the key and updates the cache.
   ...
   */
  public <T> T getWithLock(String cacheKey, long ttlSeconds, Supplier<T> dataSupplier, double jitterPercent) {
    ...
    try {
      ...
      if (redisLockUtil.tryLock(lockKey)) {
        lockValue = redisLockUtil.waitForLock(lockKey);
        if (lockValue == null) {
          log.warn("Failed to acquire lock for key: {}", cacheKey);
          // Fallback to direct data fetch without caching
